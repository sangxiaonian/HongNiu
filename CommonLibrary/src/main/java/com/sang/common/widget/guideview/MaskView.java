        ...
        switch (targetParentPosition) {
            ...
            case LayoutParams.PARENT_CENTER:
                ...
                if (rect.right > getMeasuredWidth()) {
                    rect.left -= rect.right - getMeasuredWidth();
...
//        int center = getWidth() / 2;
        int center = (int) ((mTargetRect.left + mTargetRect.right) / 2);
        int width = DeviceUtils.dip2px(getContext(), 20);

        if (center + width / 2>child.getRight()){
            center=child.getRight()-width/2-width/4;
        }

        LayoutParams layoutParams = (LayoutParams) child.getLayoutParams();

        if (layoutParams.targetAnchor== LayoutParams.ANCHOR_TOP){
            ...
            mPath.moveTo(center - width / 2, child.getTop());
            mPath.lineTo(center + width / 2, child.getTop());
            mPath.lineTo(center, child.getTop() - width / 2);
        }
        mPath.close();
